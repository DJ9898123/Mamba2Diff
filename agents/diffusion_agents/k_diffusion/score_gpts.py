...
import einops
import hydra
from beso.agents.diffusion_agents.k_diffusion.activation import *

"""
   这部分导入的包是为了引入bimamba(官方模块）
"""
...
def init_rate1_index(tensor):
    if tensor is not None:
        tensor.data.fill_(0.5)


def init_rate2_index(tensor):
    if tensor is not None:
        tensor.data.fill_(0.5)
...
class BiMambaEncoder(nn.Module):
    def __init__(
        ...
        init_rate1_index(self.rate1)

        self.rate2 = torch.nn.Parameter(torch.Tensor(1))
        init_rate2_index(self.rate2)

        # self.rate1 = nn.Parameter(torch.full((1, d_model), 0.5))
        # self.rate2 = nn.Parameter(torch.full((1, d_model), 0.5))
    ...
    def forward(self, x):
        ...
        # mamba2和attention的混合输出
        # mamba_out = residual + mamba_out_forward * self.rate2 + x_att * self.rate1
        # mamba_out = residual + mamba_out_forward
        mamba_out = residual + x_att
