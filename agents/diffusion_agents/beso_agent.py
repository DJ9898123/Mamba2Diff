...
class BesoAgent(BaseAgent):
    ...
    def load_preweight_model(self, weights_path: str, sv_name=None) -> None:
        """
        Method to load a pretrained model weights inside self.model
        """
        if sv_name is None:
            model_dict = self.model.state_dict()
            pretrained_dict = torch.load(os.path.join(weights_path, "model_state_dict.pth"), map_location=self.device)
            ...
            model_dict = self.model.state_dict()
            pretrained_dict = torch.load(os.path.join(weights_path, sv_name), map_location=self.device)
            load_key, no_load_key, temp_dict = [], [], {}
            for k, v in pretrained_dict.items():
                if k in model_dict.keys() and np.shape(model_dict[k]) == np.shape(v):
                    temp_dict[k] = v
        ...
        log.info('Loaded pre-weight model parameters for training')
