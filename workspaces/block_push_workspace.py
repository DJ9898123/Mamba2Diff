...
class BlockPushingManager(BaseWorkspaceManger):
    ...
    def test_agent(
        ...
        store_video=True,
        video_path='/home/zgp/视频'  # Path to store the generated video
        ):
        """
        ...
        for goal_idx in range(self.eval_n_times):
            ...
            # if store_video:
            # video_path = '/home/zgp/视频'
            # video_filename = f"rollout_{goal_idx}.mp4"
            # video_filepath = os.path.join(video_path, video_filename)

            # Save the frames as a video using imageio
            # 使用 imageio 和 FFMPEG 保存高质量视频
            # writer = imageio.get_writer(
            #     video_filepath, fps=30, codec="libx264", format="ffmpeg", ffmpeg_params=["-b:v", "10000k"]
            # )
            # for frame in frames:
